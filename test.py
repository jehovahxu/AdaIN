...
from function import adaptive_instance_normalization_patch
...
def style_transfer(vgg, decoder, content, style, alpha=1.0,
    ...
    if interpolation_weights:
        ...
        base_feat = adaptive_instance_normalization_patch(content_f, style_f)
        for i, w in enumerate(interpolation_weights):
            feat = feat + w * base_feat[i:i + 1]
        content_f = content_f[0:1]
    else:
        feat1 = adaptive_instance_normalization(content_f, style_f)
        feat = adaptive_instance_normalization_patch(content_f, style_f)
...
for content_path in content_paths:
    if do_interpolation:  # one content image, N style image
        ...
        for style_path in style_paths:
            ...
            out_img = torch.FloatTensor(3, 3, 640, 512)
            out_img[0, :, :, :] = content
            out_img[1, :, :, :] = style
            out_img[2, :, :, :] = output[:, 0, :, :].expand(1, 3, 640, 512)
            # out_img[2, 0, :, :] = output[:, 0, :, :]
            # out_img[2, 1, :, :] = output[:, 0, :, :]
            # out_img[2, 2, :, :] = output[:, 0, :, :]

            save_image(out_img, output_name)
            print ('save img : %s') % output_name
